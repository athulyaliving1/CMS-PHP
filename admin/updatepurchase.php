...
$status=$_POST['status'];
$remark=$_POST['remark'];
$query=mysqli_query($con,"insert into reqremark(reqNumber,status,remark) values('$complaintnumber','$status','$remark')");
$sql=mysqli_query($con,"update purchase set status='$status' where reqNumber='$complaintnumber'");
...
$query1=mysqli_query($con,"select purchase.*,users.fullName as name,users.userEmail as eml,category.categoryName as catname from purchase join users on users.id=purchase.userId join category on category.id=purchase.category where purchase.reqNumber='".$_GET['cid']."'");
